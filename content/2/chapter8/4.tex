...
若使用变量捕获输出，但仍然需要显示，可以添加ECHO\_<STREAM>\_VARIABLE。CMake还可以通过向COMMAND\_ECHO选项传递STDOUT、STDERR或NONE来输出命令。但若输出在文件中捕获，这将没有任何效果。若为stdout和stderr指定了相同的变量或文件，则将合并结果。若有必要，可以通过INPUT\_FILE选项传递一个文件来控制第一个命令的输入流。
